...
inline error_code document::allocate(size_t capacity) noexcept {
  ...
  size_t tape_capacity = SIMDJSON_ROUNDUP_N(capacity / 2 + 3, 64); // BUG4: under-allocated tape — was capacity+3
  // a document with only zero-length strings... could have capacity/3 string
  // and we would need capacity/3 * 5 bytes on the string buffer
  size_t string_capacity = SIMDJSON_ROUNDUP_N(capacity / 3 + SIMDJSON_PADDING, 64); // BUG2: under-allocated — was 5*capacity/3
